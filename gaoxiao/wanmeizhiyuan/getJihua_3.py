...
def change_grade(IP_grade):
    ...
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='xxx',
                           password='jxx', db='zhiyuan', charset='utf8')
    cursor = conn.cursor()
...
def getList():
    ...
    while list_t.__len__() < number:
        try:
            list_t = []
            # IP_grade = {}
            conn = pymysql.connect(host='172.0.0.1', port=3306,
                                   user='xxx',
                                   password='xxx', db='zhiyuan',
                                   charset='utf8')
            cursor = conn.cursor()
            qi = ci * number * 10
            zhi = number * 10
            sql = "SELECT * FROM IPproxy1 ORDER BY grade DESC LIMIT %s, %s"
